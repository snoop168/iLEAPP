...
def get_intents(files_found, report_folder, seeker):
    ...
    report.start_artifact_report(report_folder, 'Intents - Messages', description)
    report.add_script()
    message_data_headers = ('Received Time', 'Message', 'Direction', 'Thread ID', 'Other Party', 'Other Party Name')

    for file_found in files_found[0:2]:
            ...
            while length := int.from_bytes(file.read(4), byteorder='little'):
                ...
                if record_marker == 1:
                    ...
                    if (message['2'] == "com.apple.MobileSMS"):
                        intent, _ = blackboxprotobuf.decode_message(dpl['intent']['backingStore']['bytes'], message_typedef)
                        #print(intent)
        ...
        report.write_artifact_data_table(message_data_headers, message_data_list, file_found)
        message_data_list = []
